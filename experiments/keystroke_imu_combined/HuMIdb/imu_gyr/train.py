...
class TrainDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        anchor[0] = anchor[0][:, [1,2,3]]
        positive[0] = positive[0][:, [1,2,3]]
        negative[0] = negative[0][:, [1,2,3]]

        anchor[1] = anchor[1][:, 12:24]
        positive[1] = positive[1][:, 12:24]
        negative[1] = negative[1][:, 12:24]
...
class TripletLoss(nn.Module):
    ...
    def forward(self, anchor, positive, negative):
        ...
        if (not(anchor_out.isnan().any()) and not(positive_out.isnan().any()) and not(negative_out.isnan().any())):
            if (losses.isnan().any()):
                print("here")
...
if __name__ == "__main__":
    ...
    keystroke_l = config_data["data"]["keystroke_sequence_len"]
    imu_l = config_data["data"]["imu_sequence_len"]
    keystroke_feature_count = hyperparams['keystroke_feature_count']['humi']
    imu_feature_count = hyperparams['imu_feature_count']['one_type']
